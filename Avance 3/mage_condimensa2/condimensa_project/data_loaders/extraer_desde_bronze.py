...
def extraer_desde_bronze(*args, **kwargs):
    ...
    with Postgres.with_config(ConfigFileLoader(config_path, config_profile)) as loader:
        ...
        # Extraer: quickbooks_produccion_categorias_pp_raw
        print("[3d] Extrayendo bronze.quickbooks_produccion_categorias_pp_raw...")
        try:
            query = "SELECT * FROM bronze.quickbooks_produccion_categorias_pp_raw"
            df = loader.load(query)
            dfs['quickbooks_produccion_categorias_pp_raw'] = df
            print(f"    Registros: {len(df)}")
        except Exception as e:
            print(f"    [WARN] No se pudo extraer quickbooks_produccion_categorias_pp_raw: {e}")
