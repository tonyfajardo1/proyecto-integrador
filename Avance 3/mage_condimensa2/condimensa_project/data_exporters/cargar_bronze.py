...
def cargar_bronze(data, *args, **kwargs):
    ...
    with Postgres.with_config(ConfigFileLoader(config_path, config_profile)) as loader:
        ...
        # =====================================================================
        # 6) QUICKBOOKS STAGING: categorias PP raw
        # =====================================================================
        if 'quickbooks.stg_produccion_categorias_pp_raw' in dfs:
            print(
                "[6] Cargando quickbooks.stg_produccion_categorias_pp_raw "
                "a bronze.quickbooks_produccion_categorias_pp_raw..."
            )
            dato = dfs['quickbooks.stg_produccion_categorias_pp_raw']
            ...
            if len(df) > 0:
                loader.execute("""
                    CREATE TABLE IF NOT EXISTS bronze.quickbooks_produccion_categorias_pp_raw (
                        id SERIAL PRIMARY KEY,
                        familia TEXT,
                        categoria_pp TEXT,
                        source_file TEXT,
                ...
                cols_dest = loader.load("""
                    ...
                    FROM information_schema.columns
                    WHERE table_schema = 'bronze'
                      AND table_name = 'quickbooks_produccion_categorias_pp_raw'
                    ORDER BY ordinal_position
                """)['column_name'].tolist()
                cols_pp = [c for c in cols_dest if c != 'id']

                for c in cols_pp:
                    if c not in df.columns:
                        df[c] = None

                loader.execute("TRUNCATE TABLE bronze.quickbooks_produccion_categorias_pp_raw")
                loader.export(
                    df[cols_pp],
                    schema_name='bronze',
                    table_name='quickbooks_produccion_categorias_pp_raw',
                    if_exists='append',
                )
                print(f"    Registros cargados: {len(df)}")
                resultados['quickbooks_produccion_categorias_pp_raw'] = len(df)
    ...
    required_loaded = [
        ...
        'quickbooks_produccion_categorias_pp_raw',
    ]
    if require_kronos:
        required_loaded.append('kronos_ventas_detalle_raw')
    ...
    if 'kronos_ventas_detalle_raw' not in resultados:
        print('[WARN] Kronos no cargado en Bronze. Se continua con QuickBooks para clasificacion PP/PT.')
