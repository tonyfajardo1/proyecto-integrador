...
def _project_inventory_for_product(
    group: pd.DataFrame,
    demand_col: str = "cantidad_predicha",
    production_col: str = "cantidad_a_producir_ajustada",
    include_inventory_state: bool = True,
) -> pd.DataFrame:
    out = group.sort_values("periodo").copy()
    if demand_col not in out.columns:
        out[demand_col] = out["cantidad_predicha"]
    ...
    demands = pd.to_numeric(out[demand_col], errors="coerce").fillna(0.0)
    for row, demand in zip(out.itertuples(index=False), demands.to_numpy()):
        demand = float(demand)
    ...
    out[production_col] = production
    return out


def _project_production_scenario(
    base: pd.DataFrame,
    demand_col: str,
    production_col: str,
) -> pd.DataFrame:
    scenario = pd.concat(
        [
            _project_inventory_for_product(
                group,
                demand_col=demand_col,
                production_col=production_col,
                include_inventory_state=False,
            )
            for _, group in base.groupby("product_id", sort=False)
        ],
        ignore_index=True,
    )
    return scenario[["product_id", "periodo", production_col]]
...
def apply_inventory_adjustments(config: dict[str, Any], source: str, predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    for demand_col, production_col in [
        ("prediccion_min", "cantidad_min_a_producir_ajustada"),
        ("prediccion_max", "cantidad_max_a_producir_ajustada"),
    ]:
        scenario = _project_production_scenario(out, demand_col, production_col)
        adjusted = adjusted.merge(scenario, on=["product_id", "periodo"], how="left")
        adjusted[production_col] = adjusted[production_col].fillna(
            adjusted["cantidad_a_producir_ajustada"]
        )

    adjusted["cantidad_min_a_producir_ajustada"] = np.minimum(
        adjusted["cantidad_min_a_producir_ajustada"],
        adjusted["cantidad_a_producir_ajustada"],
    ).round(2)
    adjusted["cantidad_max_a_producir_ajustada"] = np.maximum(
        adjusted["cantidad_max_a_producir_ajustada"],
        adjusted["cantidad_a_producir_ajustada"],
    ).round(2)
