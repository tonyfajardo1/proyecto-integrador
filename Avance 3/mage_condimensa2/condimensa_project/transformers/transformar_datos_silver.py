...
def transformar_kronos_silver(data, *args, **kwargs):
    ...
    def catalog_lookup_v3(catalog):
        ...
        fuzzy_choices = sorted(
            set(catalog.get('product_norm', pd.Series(dtype=str)).dropna())
            | set(catalog.get('item_leaf_norm', pd.Series(dtype=str)).dropna())
        )
    ...
    # Catalogo de familias PP proveniente de PRODUCCION2025/Categorias.
    ...
    if 'catalogo_ean_clean' in resultados:
        ...
        if len(cat_pt) > 0:
            ...
            cat_pt['product_code'] = cat_pt['product_name'].map(extract_product_code_v3)
        ...
        if 'ventas_econespecias_mensual_clean' in resultados:
            ...
            lookup = catalog_lookup_v3(cat_pt)
            unique_products = (
