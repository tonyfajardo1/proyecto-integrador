...
def _agregar_estado_planificacion(
    ...
    out['estado_producto_plan'] = 'Activo'
    out.loc[estacional_aux & ~inactivo, 'estado_producto_plan'] = 'Estacional'
    out.loc[inactivo, 'estado_producto_plan'] = 'Inactivo'
...
def render():
    ...
    try:
        ...
        estado_sel = st.selectbox('Estado producto', ['Vigentes', 'Todos'] + estados, index=0)
        if estado_sel == 'Vigentes':
            df_plan = df[df['estado_producto_plan'] != 'Inactivo'].copy()
        ...
        tabla = tabla.rename(columns={
            ...
            'tipo_producto': 'Tipo',
            'estado_producto_plan': 'Estado producto',
        ...
        tabla = tabla.sort_values(['Tipo', 'Estado producto', 'Produccion recomendada'], ascending=[True, True, False])
        ...
        st.dataframe(
            ...
            column_config={
                ...
                'Estado producto': st.column_config.TextColumn('Estado producto', width='small'),
        ...
        with tab_est:
            df_est_tab = df[df['estado_producto_plan'] == 'Estacional'].copy()
            if len(df_est_tab) == 0:
        ...
        with tab_ina:
            df_ina_tab = df[df['estado_producto_plan'] == 'Inactivo'].copy()
            if len(df_ina_tab) == 0:
