...
def _bias_pct(y_true, y_pred):
    y_true = np.asarray(y_true, dtype=float)
    y_pred = np.asarray(y_pred, dtype=float)
    den = np.abs(y_true).sum()
    if den == 0:
        return np.nan
    return (y_pred - y_true).sum() / den
...
def _smape(y_true, y_pred):
    ...
    if mask.sum() == 0:
        return np.nan
    return np.mean(2 * np.abs(y_pred[mask] - y_true[mask]) / den[mask])
...
def _metrics(y_true, y_pred):
    ...
    return {
        ...
        "BIAS_PCT": _bias_pct(y_true, y_pred),
...
def _build_plan_comparison_reports(pred_table: pd.DataFrame):
    ...
    m_plan = {
        ...
        "BIAS_PCT": np.nan,
        "SMAPE": np.nan,
    } if plan_non_informative_global else _comparison_metrics(comp, "qty_planificada_t1")
        ...
        [
            {
                ...
                "BIAS_PCT_modelo": m_model["BIAS_PCT"],
                "SMAPE_modelo": m_model["SMAPE"],
                "WAPE_planificada": m_plan["WAPE"],
                "BIAS_PCT_planificada": m_plan["BIAS_PCT"],
                "SMAPE_planificada": m_plan["SMAPE"],
    ...
    for tipo, sub in comp.groupby("tipo_producto", dropna=False):
        ...
        mp = {
            ...
            "BIAS_PCT": np.nan,
            "SMAPE": np.nan,
        } if plan_non_informative else _comparison_metrics(sub, "qty_planificada_t1")
        by_tipo_rows.append(
            {
                "tipo_producto": tipo,
                "n_rows": len(sub),
                ...
                "WAPE_modelo": mm["WAPE"],
                "BIAS_PCT_modelo": mm["BIAS_PCT"],
                "SMAPE_modelo": mm["SMAPE"],
                "WAPE_planificada": mp["WAPE"],
                "BIAS_PCT_planificada": mp["BIAS_PCT"],
                "SMAPE_planificada": mp["SMAPE"],
...
def run_full_modeling(config: ModelingConfig | None = None):
    ...
    rows.append({
        ...
        "BIAS_PCT_train": m_train["BIAS_PCT"],
        "SMAPE_train": m_train["SMAPE"],
        "MAE_val": m_val["MAE"],
        "RMSE_val": m_val["RMSE"],
        "WAPE_val": m_val["WAPE"],
        "BIAS_PCT_val": m_val["BIAS_PCT"],
        "SMAPE_val": m_val["SMAPE"],
        "MAE_test": m_test["MAE"],
    ...
    benchmark["ABS_BIAS_PCT_val"] = benchmark["BIAS_PCT_val"].abs()
    benchmark["ABS_BIAS_PCT_test"] = benchmark["BIAS_PCT_test"].abs()
    benchmark = benchmark.sort_values(
        ["WAPE_val", "ABS_BIAS_PCT_val", "WAPE_test", "ABS_BIAS_PCT_test"]
