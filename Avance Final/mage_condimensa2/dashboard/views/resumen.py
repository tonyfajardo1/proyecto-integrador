...
from database import (
    get_resumen_ejecutivo_kronos,
    get_kpi_ventas,
    get_kpi_ventas_detalle,
    get_alertas,
    get_combinaciones,
    get_predicciones,
)
...
def _filtrar_proximo_periodo_por_tipo(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tipo_col = 'tipo_producto' if 'tipo_producto' in out.columns else 'fuente_modelo'
...
def render():
    ...
    **Fuentes de datos:** Kronos (comercial) y QuickBooks (produccion).
    ...
    try:
        df_kpi_detalle = get_resumen_ejecutivo_kronos()
        df_kpi = pd.DataFrame()

        if len(df_kpi_detalle) > 0:
            for col in ['total_venta', 'total_neto', 'total_devolucion', 'rentabilidad', 'cant_venta', 'cant_neto']:
            ...
            if len(df_filtrado) > 0:
                ...
                if 'cant_neto' in df_filtrado.columns:
                    df_kpi['cant_neto'] = df_filtrado.groupby('centro_costo')['cant_neto'].sum().values
                    df_kpi['ticket_promedio'] = (
                        df_kpi['total_neto'] / df_kpi['cant_neto'].replace(0, pd.NA)
                    ).fillna(0)
                else:
        ...
        with col3:
            ...
            if len(df_pred) > 0:
                df_pred_operativo = _filtrar_proximo_periodo_por_tipo(df_pred)
                df_pred_operativo['qty_recomendada'] = pd.to_numeric(
                    df_pred_operativo['qty_recomendada'],
                    errors='coerce',
                ).fillna(0)
                st.metric("Productos con pronostico", df_pred_operativo['producto'].nunique())
                st.metric("Qty recomendada total", f"{df_pred_operativo['qty_recomendada'].sum():,.0f}")

                top = df_pred_operativo.sort_values('qty_recomendada', ascending=False).head(3)
