...
def _numeric_with_missing(series: pd.Series) -> tuple[pd.Series, pd.Series]:
    parsed = pd.to_numeric(series, errors="coerce")
    missing = parsed.isna().astype(int)
    return parsed.fillna(0.0), missing
...
def _complete_monthly_grid(
    ...
    observed["monthly_observed_flag"] = 1
    out = grid.merge(observed, on=["product_id", "periodo"], how="left")
    ...
    numeric_cols = [
        ...
        if col not in {"product_id", "periodo"} and pd.api.types.is_numeric_dtype(out[col])
    ]
    for col in numeric_cols:
        out[col] = out[col].fillna(0.0)

    out["monthly_observed_flag"] = out["monthly_observed_flag"].fillna(0).astype(int)
...
def build_pt_dataset(config: dict[str, Any], catalog: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df["cantidad"], df["cantidad_is_missing"] = _numeric_with_missing(df["cantidad"])
    df["ventas"], df["ventas_is_missing"] = _numeric_with_missing(df["ventas"])
    df["recuento_cliente"], df["recuento_cliente_is_missing"] = _numeric_with_missing(df["recuento_cliente"])
    ...
    monthly = (
        ...
        .agg(
            ...
            missing_cantidad_rows=("cantidad_is_missing", "sum"),
            missing_ventas_rows=("ventas_is_missing", "sum"),
            missing_recuento_cliente_rows=("recuento_cliente_is_missing", "sum"),
        )
    )
    monthly["target_qty_raw"] = monthly["target_qty"]
    monthly["target_qty"] = monthly["target_qty"].clip(lower=0.0)
    monthly["missing_cantidad_flag"] = monthly["missing_cantidad_rows"].gt(0).astype(int)
    monthly["missing_ventas_flag"] = monthly["missing_ventas_rows"].gt(0).astype(int)
    monthly["missing_recuento_cliente_flag"] = monthly["missing_recuento_cliente_rows"].gt(0).astype(int)
    monthly["target_qty_missing_source_flag"] = monthly["missing_cantidad_flag"]
    monthly["target_qty_proxy_flag"] = 0
    ...
    grid = _complete_monthly_grid(
        ...
        [
            ...
            "ventas",
            "recuento_cliente",
            "productos_raw_distintos",
            "missing_cantidad_rows",
            "missing_ventas_rows",
            "missing_recuento_cliente_rows",
...
def build_pp_dataset(config: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df["q_planificada"], df["q_planificada_is_missing"] = _numeric_with_missing(df["q_planificada"])
    df["q_liberada"], df["q_liberada_is_missing"] = _numeric_with_missing(df["q_liberada"])
    df["q_fabricada"], df["q_fabricada_is_missing"] = _numeric_with_missing(df["q_fabricada"])
    use_fabricada = df["q_fabricada"].gt(0)
    use_liberada = ~use_fabricada & df["q_liberada"].gt(0)
    use_planificada = ~use_fabricada & ~use_liberada & df["q_planificada"].gt(0)
    df["target_qty"] = df["q_fabricada"].where(use_fabricada, df["q_liberada"])
    df["target_qty"] = df["target_qty"].where(use_fabricada | use_liberada, df["q_planificada"]).fillna(0.0)
    df["target_qty_proxy_flag"] = (use_liberada | use_planificada).astype(int)
    df["target_qty_missing_source_flag"] = (
        df["q_planificada_is_missing"].eq(1)
        & df["q_liberada_is_missing"].eq(1)
        & df["q_fabricada_is_missing"].eq(1)
    ).astype(int)
    ...
    monthly = (
        ...
        .agg(
            ...
            missing_q_planificada_rows=("q_planificada_is_missing", "sum"),
            missing_q_liberada_rows=("q_liberada_is_missing", "sum"),
            missing_q_fabricada_rows=("q_fabricada_is_missing", "sum"),
            target_qty_proxy_rows=("target_qty_proxy_flag", "sum"),
            target_qty_missing_source_rows=("target_qty_missing_source_flag", "sum"),
        )
    )
    monthly["target_qty_raw"] = monthly["target_qty"]
    monthly["target_qty"] = monthly["target_qty"].clip(lower=0.0)
    monthly["missing_q_planificada_flag"] = monthly["missing_q_planificada_rows"].gt(0).astype(int)
    monthly["missing_q_liberada_flag"] = monthly["missing_q_liberada_rows"].gt(0).astype(int)
    monthly["missing_q_fabricada_flag"] = monthly["missing_q_fabricada_rows"].gt(0).astype(int)
    monthly["target_qty_proxy_flag"] = monthly["target_qty_proxy_rows"].gt(0).astype(int)
    monthly["target_qty_missing_source_flag"] = monthly["target_qty_missing_source_rows"].gt(0).astype(int)
    product_meta = (
    ...
    grid = _complete_monthly_grid(
        ...
        [
            ...
            "lotes",
            "ordenes",
            "missing_q_planificada_rows",
            "missing_q_liberada_rows",
            "missing_q_fabricada_rows",
            "missing_q_planificada_flag",
            "missing_q_liberada_flag",
            "missing_q_fabricada_flag",
            "target_qty_proxy_rows",
            "target_qty_proxy_flag",
            "target_qty_missing_source_rows",
            "target_qty_missing_source_flag",
        ],
    )
    product_meta = product_meta[product_meta["product_id"].isin(grid["product_id"].unique())].copy()
